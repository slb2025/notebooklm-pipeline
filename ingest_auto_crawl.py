...
LOG_FILE = Path("processed_urls.log")

MIN_YEAR = 2022
MAX_PAGES_PER_SITE = 50  # Lowered slightly for full browser loading time
MAX_CONCURRENT_PAGES = 1 # Keep it serial for now to be polite and avoid detection

# Keywords for discovery
KEYWORDS = ["/blog/", "/research/", "/engineering/", "/white-paper/", "/news/", "/post/", "/publications/", "/articles/"]
...
async def process_site(page, base_url, processed_urls):
    ...
    try:
        ...
        candidates = set()
        # Patterns to SKIP (Social media, pagination, archives, feeds, labels)
        SKIP_PATTERNS = [
